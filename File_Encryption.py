...
def encrypt_file(input_path, output_path, key):
    with open(input_path, "rb") as f_in:
        data = f_in.read() # read the data from the file and assign it to the variable: data
    scrambled = scramble(data, key) # transposition the data with the key value input
    with open(output_path, "wb") as f_out:
        f_out.write(scrambled) # save the scrambled data into a file
    print(f"File encrypted and saved as {output_path}")

def decrypt_file(input_path, output_path, key):
    with open(input_path, "rb") as f_in:
        data = f_in.read() # read the scrambled data from the file
    unscrambled = unscramble(data, key) # reverse the scrambling process
    with open(output_path, "wb") as f_out:
        f_out.write(unscrambled) # output the unscrambled data into a file
